...
${formatHelpExamples([
  ['selfer agent --to +15555550123 --message "status update"', "Start a new session."],
  ['selfer agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'selfer agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'selfer agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['selfer agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'selfer agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.selfer.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.selfer.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['selfer agents set-identity --agent main --name "Selfer" --emoji "🦞"', "Set name + emoji."],
  ["selfer agents set-identity --agent main --avatar avatars/selfer.png", "Set avatar path."],
  [
    "selfer agents set-identity --workspace ~/.selfer/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "selfer agents set-identity --identity-file ~/.selfer/workspace/IDENTITY.md --agent main",
