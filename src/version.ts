...
declare const __Selfer_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "Selfer";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["Selfer_VERSION"],
      runtimeVersion,
      env["Selfer_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Selfer version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __Selfer_VERSION__ === "string" ? __Selfer_VERSION__ : undefined,
  bundledVersion: process.env.Selfer_BUNDLED_VERSION,
});
