...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const SelferTmpDir = path.resolve(resolvePreferredSelferTmpDir());
  if (!isPathInside(SelferTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: SelferTmpDir });
  return resolved;
}
