...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SELFER_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*selfer-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "selfer";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "selfer");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "selfer-gw");
  ...
  records.push(`_selfer-gw._tcp IN PTR ${instanceLabel}._selfer-gw._tcp`);
  records.push(`${instanceLabel}._selfer-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._selfer-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; selfer-content-hash: ${contentHash}\n${contentBody}`;
}
