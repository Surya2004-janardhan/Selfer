...
export async function loadValidConfigOrThrow(): Promise<SelferConfig> {
...
export async function updateConfig(
  mutator: (cfg: SelferConfig) => SelferConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: SelferConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: SelferConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("Selfer agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
