    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["selfer memory status", "Show index and provider status."],
          ["selfer memory index --force", "Force a full reindex."],
          ['selfer memory search --query "deployment notes"', "Search indexed memory entries."],
          ["selfer memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.selfer.ai/cli/memory")}\n`,
    );
