...
import type {
  SelferPluginApi,
  SelferPluginChannelRegistration,
  SelferPluginCliRegistrar,
  SelferPluginCommandDefinition,
  SelferPluginHttpRouteAuth,
  SelferPluginHttpRouteMatch,
  SelferPluginHttpRouteHandler,
  SelferPluginHttpRouteParams,
  SelferPluginHookOptions,
  ProviderPlugin,
  SelferPluginService,
  SelferPluginToolContext,
  SelferPluginToolFactory,
  PluginConfigUiHint,
  PluginDiagnostic,
  PluginLogger,
  PluginOrigin,
...
export type PluginToolRegistration = {
  pluginId: string;
  factory: SelferPluginToolFactory;
  names: string[];
  optional: boolean;
  source: string;
};
...
export type PluginCliRegistration = {
  pluginId: string;
  register: SelferPluginCliRegistrar;
  commands: string[];
  source: string;
};
...
export type PluginServiceRegistration = {
  pluginId: string;
  service: SelferPluginService;
  source: string;
};

export type PluginCommandRegistration = {
  pluginId: string;
  command: SelferPluginCommandDefinition;
  source: string;
};
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: SelferPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: SelferPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: SelferPluginHookOptions | undefined,
    config: SelferPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: SelferPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: SelferPluginChannelRegistration | ChannelPlugin,
  ) => {
    const normalized =
      typeof (registration as SelferPluginChannelRegistration).plugin === "object"
        ? (registration as SelferPluginChannelRegistration)
  ...
  const registerCli = (
    record: PluginRecord,
    registrar: SelferPluginCliRegistrar,
  ...
  const registerService = (record: PluginRecord, service: SelferPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: SelferPluginCommandDefinition) => {
