      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["selfer gateway run", "Run the gateway in the foreground."],
            ["selfer gateway status", "Show service status and probe reachability."],
            ["selfer gateway discover", "Find local and wide-area gateway beacons."],
            ["selfer gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.selfer.ai/cli/gateway")}\n`,
      ),
  );
