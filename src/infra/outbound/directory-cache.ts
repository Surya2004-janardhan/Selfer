...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: SelferConfig | null = null;
  ...
  get(key: string, cfg: SelferConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: SelferConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: SelferConfig): void {
