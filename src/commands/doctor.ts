...
function resolveMode(cfg: SelferConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveSelferPackageRoot({
  ...
  let cfg: SelferConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("Selfer configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("Selfer config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("Selfer setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("Selfer config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("Selfer config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("Selfer doctor --fix")}" to apply changes.`);
  }
