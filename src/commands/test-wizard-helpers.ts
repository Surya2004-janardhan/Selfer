...
export async function setupAuthTestEnv(
  ...
  process.env.Selfer_STATE_DIR = stateDir;
  process.env.Selfer_AGENT_DIR = agentDir;
...
export function requireSelferAgentDir(): string {
  const agentDir = process.env.Selfer_AGENT_DIR;
  if (!agentDir) {
    throw new Error("Selfer_AGENT_DIR not set");
  }
  return agentDir;
}
