...
export const POSIX_SELFER_TMP_DIR = "/tmp/selfer";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredSelferTmpDirOptions = {
...
export function resolvePreferredSelferTmpDir(
  options: ResolvePreferredSelferTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "selfer" : `selfer-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[selfer] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Selfer temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Selfer temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_SELFER_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_SELFER_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_SELFER_TMP_DIR)) {
      return POSIX_SELFER_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_SELFER_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_SELFER_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_SELFER_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_SELFER_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_SELFER_TMP_DIR;
