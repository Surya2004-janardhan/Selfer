...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".Selfer");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.Selfer_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$Selfer_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via Selfer_STATE_DIR.
