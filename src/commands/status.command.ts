...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Selfer status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`Selfer devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("Selfer devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("Selfer devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("Selfer security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("Selfer security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.Selfer.ai/faq");
  runtime.log("Troubleshooting: https://docs.Selfer.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("Selfer status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("Selfer logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("Selfer status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("Selfer gateway probe")}`);
  }
}
