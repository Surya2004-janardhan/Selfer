...
const GOOGLE_LIVE_SINGLE_KEY = "Selfer_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "Selfer_LIVE_ANTHROPIC_KEY",
    listVar: "Selfer_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "Selfer_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `Selfer_LIVE_${base}_KEY`;
