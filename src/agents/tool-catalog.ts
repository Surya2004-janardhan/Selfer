...
function buildCoreToolGroupMap() {
  ...
  const SelferTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInSelferGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:Selfer": SelferTools,
