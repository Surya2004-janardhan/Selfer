...
import { readConfigFileSnapshot, writeConfigFile, type SelferConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("Selfer config set gateway.auth.mode token")} or ${formatCliCommand("Selfer config set gateway.auth.mode password")}.`,
  ].join(" ");
}
...
export async function resolveGatewayInstallToken(
  ...
  const envToken =
    options.env.Selfer_GATEWAY_TOKEN?.trim() || options.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
