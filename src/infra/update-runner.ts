...
const DEFAULT_PACKAGE_NAME = "selfer";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["selfer", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "selfer-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "selfer.mjs");
    ...
    const doctorStep = await runStep(
      step("selfer doctor", doctorArgv, gitRoot, { SELFER_UPDATE_IN_PROGRESS: "1" }),
    );
