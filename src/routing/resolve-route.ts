...
function listAgents(cfg: SelferConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: SelferConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<SelferConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: SelferConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: SelferConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: SelferConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<SelferConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  SelferConfig,
  {
    bindingsRef: SelferConfig["bindings"];
    agentsRef: SelferConfig["agents"];
    sessionRef: SelferConfig["session"];
...
function resolveRouteCacheForConfig(cfg: SelferConfig): Map<string, ResolvedAgentRoute> {
