...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "Selfer_STATE_DIR",
  "Selfer_CONFIG_PATH",
  "Selfer_SKIP_BROWSER_CONTROL_SERVER",
  "Selfer_SKIP_GMAIL_WATCHER",
  "Selfer_SKIP_CANVAS_HOST",
  "Selfer_BUNDLED_PLUGINS_DIR",
  "Selfer_SKIP_CHANNELS",
  "Selfer_SKIP_PROVIDERS",
  "Selfer_SKIP_CRON",
  "Selfer_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "Selfer-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.Selfer_STATE_DIR = path.join(tempHome, ".Selfer");
  delete process.env.Selfer_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.Selfer_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.Selfer_SKIP_GMAIL_WATCHER = "1";
  process.env.Selfer_SKIP_CANVAS_HOST = "1";
  process.env.Selfer_SKIP_CHANNELS = "1";
  process.env.Selfer_SKIP_PROVIDERS = "1";
  process.env.Selfer_SKIP_CRON = "1";
  process.env.Selfer_TEST_MINIMAL_GATEWAY = "1";
  process.env.Selfer_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "Selfer-test-no-bundled-extensions")
    : "Selfer-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".Selfer-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".Selfer-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__SelferTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__SelferTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["Selfer_GATEWAY_TOKEN"]);
  const prev = process.env.Selfer_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    delete process.env.Selfer_GATEWAY_TOKEN;
  } else {
    process.env.Selfer_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.Selfer_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
