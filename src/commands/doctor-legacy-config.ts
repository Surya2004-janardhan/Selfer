...
export function normalizeCompatibilityConfigValues(cfg: SelferConfig): {
  config: SelferConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: SelferConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as SelferConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as SelferConfig["browser"],
    };
    changes.push(
