    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "selfer node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["selfer node status", "Check node host service status."],
          ["selfer node install", "Install the node host service."],
          ["selfer node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.selfer.ai/cli/node")}\n`,
    );
