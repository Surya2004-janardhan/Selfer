...
import { resolveSelferAgentDir } from "../../agents/agent-paths.js";
import {
  resolveAgentDir,
...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveSelferAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("Selfer models auth setup-token")}\` or \`${formatCliCommand("Selfer configure")}\`.`
          : `Run \`${formatCliCommand("Selfer configure")}\` or set an API key env var.`;
