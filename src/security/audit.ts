...
import type { ConfigFileSnapshot, SelferConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: SelferConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Selfer state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Selfer state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.Selfer_GATEWAY_TOKEN) || hasNonEmptyString(env.CLAWDBOT_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.Selfer_GATEWAY_PASSWORD) ||
...
function collectLoggingFindings(cfg: SelferConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: SelferConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: SelferConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("Selfer status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("Selfer security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set Selfer_GATEWAY_TOKEN/Selfer_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("Selfer security audit --deep")}".`,
    });
  }
