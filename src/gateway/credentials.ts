  ...
  constructor(path: string) {
    super(
      [
        ...
        "Fix: set Selfer_GATEWAY_TOKEN/Selfer_GATEWAY_PASSWORD, pass explicit --token/--password,",
...
function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.Selfer_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
...
function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.Selfer_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
