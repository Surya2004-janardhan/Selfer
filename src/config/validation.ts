...
import type { SelferConfig, ConfigValidationIssue } from "./types.js";
import { SelferSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: SelferConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: SelferConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: SelferConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = SelferSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as SelferConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as SelferConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as SelferConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as SelferConfig,
  };
}
