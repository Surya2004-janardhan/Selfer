...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __selferDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__selferDiagnosticEventsState) {
    globalStore.__selferDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__selferDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: SelferConfig): boolean {
