...
export function isCommandEnabled(cfg: SelferConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: SelferConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as SelferConfig),
...
export function getCommandDetection(_cfg?: SelferConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: SelferConfig) {
