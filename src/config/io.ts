...
import type { SelferConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareSelferVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "SELFER_GATEWAY_TOKEN",
  "SELFER_GATEWAY_PASSWORD",
];
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  selfer config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  selfer config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: SelferConfig,
  pathSegments: string[],
): { changed: boolean; next: SelferConfig } {
...
function coerceConfig(value: unknown): SelferConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as SelferConfig;
}
...
function stampConfigVersion(cfg: SelferConfig): SelferConfig {
...
function warnIfConfigFromFuture(cfg: SelferConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareSelferVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Selfer (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as SelferConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): SelferConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as SelferConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: SelferConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as SelferConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.SELFER_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.SELFER_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.SELFER_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.SELFER_WATCH_SESSION === "string" &&
        deps.env.SELFER_WATCH_SESSION.trim().length > 0
          ? deps.env.SELFER_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.SELFER_WATCH_COMMAND === "string" &&
        deps.env.SELFER_WATCH_COMMAND.trim().length > 0
          ? deps.env.SELFER_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `SELFER_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: SelferConfig | null = null;
let runtimeConfigSourceSnapshot: SelferConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.SELFER_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.SELFER_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): SelferConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): SelferConfig {
