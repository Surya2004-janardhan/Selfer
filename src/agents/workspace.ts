...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.Selfer_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".Selfer", `workspace-${profile}`);
  }
  return path.join(home, ".Selfer", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".Selfer";
