...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    SelferHome: process.env.Selfer_HOME,
    stateDir: process.env.Selfer_STATE_DIR,
  };
}
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("Selfer_HOME", snapshot.SelferHome);
  restoreKey("Selfer_STATE_DIR", snapshot.stateDir);
}

function setTempHomeEnv(home: string): void {
  process.env.HOME = home;
  process.env.USERPROFILE = home;
  delete process.env.Selfer_HOME;
  process.env.Selfer_STATE_DIR = join(home, ".Selfer");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "Selfer-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".Selfer", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): SelferConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "Selfer"),
