...
import { scheduleGatewaySigusr1Restart, triggerSelferRestart } from "../../infra/restart.js";
...
export const handleSessionCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  const maxAgeExpiresAt =
    maxAgeMs > 0 ? telegramBinding!.boundAt + maxAgeMs : undefined;
  ...
  if (!durationArgRaw) {
    if (action === SESSION_ACTION_IDLE) {
      if (
        ...
        return {
          shouldContinue: false,
          reply: {
            text: `ℹ️ Idle timeout active (next auto-unfocus at ${formatSessionExpiry(idleExpiresAt)}).`,
          },
        };
      }
      return {
    ...
    if (
      ...
      return {
        shouldContinue: false,
        reply: {
          text: `ℹ️ Max age active (hard auto-unfocus at ${formatSessionExpiry(maxAgeExpiresAt)}).`,
        },
      };
    }
    return {
  ...
  return {
    ...
    reply: {
      text:
        action === SESSION_ACTION_IDLE
          ? `✅ Idle timeout set for ${updatedBindings.length} binding${updatedBindings.length === 1 ? "" : "s"} (next auto-unfocus at ${expiryLabel}).`
          : `✅ Max age set for ${updatedBindings.length} binding${updatedBindings.length === 1 ? "" : "s"} (hard auto-unfocus at ${expiryLabel}).`,
    },
  };
};
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Selfer in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerSelferRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Selfer via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
