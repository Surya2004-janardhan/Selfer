...
async function runGatewayHealthCheck(params: {
  ...
  const token =
    process.env.Selfer_GATEWAY_TOKEN ?? process.env.CLAWDBOT_GATEWAY_TOKEN ?? configuredToken;
  const password =
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from Selfer.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Selfer update wizard" : "Selfer configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: SelferConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("Selfer doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
