...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.Selfer_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("Selfer gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
