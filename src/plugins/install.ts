...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing Selfer.extensions; update the plugin package to include Selfer.extensions (for example ["./dist/index.js"]). See https://docs.Selfer.ai/help/troubleshooting#plugin-install-fails-with-missing-Selfer-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_Selfer_EXTENSIONS: "missing_Selfer_extensions",
  EMPTY_Selfer_EXTENSIONS: "empty_Selfer_extensions",
...
function ensureSelferExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_Selfer_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json Selfer.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_Selfer_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureSelferExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from Selfer.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-Selfer"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "Selfer security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "Selfer security audit --deep" after install.`,
    );
  }
