...
import type { SelferConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: SelferConfig): string[] {
...
export async function noteSecurityWarnings(cfg: SelferConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("Selfer security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.Selfer/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("Selfer approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.Selfer.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("Selfer configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("Selfer config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("Selfer doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "Selfer config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("Selfer config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('Selfer config set session.dmScope "per-channel-peer"') +
