...
const SEARCH_TOOL = "https://docs.Selfer.ai/mcp.SearchSelfer";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.Selfer.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('Selfer docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.Selfer.ai/");
      runtime.log(`Search: ${formatCliCommand('Selfer docs "your query"')}`);
    }
    return;
  }
