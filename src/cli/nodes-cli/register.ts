    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["selfer nodes status", "List known nodes with live status."],
          ["selfer nodes pairing pending", "Show pending node pairing requests."],
          ['selfer nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["selfer nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.selfer.ai/cli/nodes")}\n`,
    );
