...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Selfer and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["selfer update", "Update a source checkout (git)"],
        ["selfer update --channel beta", "Switch to beta channel (git + npm)"],
        ["selfer update --channel dev", "Switch to dev channel (git + npm)"],
        ["selfer update --tag beta", "One-off update to a dist-tag or version"],
        ["selfer update --dry-run", "Preview actions without changing anything"],
        ["selfer update --no-restart", "Update without restarting the service"],
        ["selfer update --json", "Output result as JSON"],
        ["selfer update --yes", "Non-interactive (accept downgrade prompts)"],
        ["selfer update wizard", "Interactive update wizard"],
        ["selfer --update", "Shorthand for selfer update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run selfer update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.selfer.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["selfer update status", "Show channel + version status."],
          ["selfer update status --json", "JSON output."],
          ["selfer update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.selfer.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
