...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.SELFER_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.SELFER_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set SELFER_APNS_TEAM_ID and SELFER_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.SELFER_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.SELFER_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.SELFER_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set SELFER_APNS_PRIVATE_KEY_P8 or SELFER_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading SELFER_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createSelferPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    selfer: createSelferPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
