...
import { isChannelAccountEffectivelyEnabled, isEnabledFlag, type ResolverContext, type SecretDefaults, collectSecretInputAssignment, hasOwnProperty } from "./runtime-shared.js";
  ...
  const topLevelBotTokenActive = !surface.channelEnabled
    ...
    : !surface.hasExplicitAccounts
      ...
      : surface.accounts.some(({ account, enabled }) => {
          ...
          const accountBotToken = account.botToken;
          const accountBotTokenConfigured = typeof accountBotToken === "string" && accountBotToken.trim().length > 0;
...
export function collectChannelSecretAssignments(params: {
  config: SelferConfig;
  defaults: SecretDefaults | undefined;
  context: ResolverContext;
}): void {
