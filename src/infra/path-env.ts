...
function candidateBinDirs(opts: EnsureSelferPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `selfer` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "selfer");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.SELFER_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "selfer"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `selfer` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSelferCliOnPath(opts: EnsureSelferPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SELFER_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.SELFER_PATH_BOOTSTRAPPED = "1";
