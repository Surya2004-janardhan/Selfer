...
export function triggerSelferRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.SELFER_SYSTEMD_UNIT,
        process.env.SELFER_PROFILE,
      );
  ...
  const label =
    process.env.SELFER_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.SELFER_PROFILE);
