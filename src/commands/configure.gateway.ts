...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "Selfer_GATEWAY_TOKEN",
          placeholder: "Selfer_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like Selfer_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Selfer will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Selfer trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.Selfer.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
