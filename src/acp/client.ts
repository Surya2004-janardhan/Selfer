...
export function resolveAcpClientSpawnEnv(
  ...
  return { ...baseEnv, Selfer_SHELL: "acp-client" };
}
...
export async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  const serverCommand = opts.serverCommand ?? (entryPath ? process.execPath : "Selfer");
  ...
  await client.initialize({
    ...
    clientInfo: { name: "Selfer-acp-client", version: "1.0.0" },
  });
