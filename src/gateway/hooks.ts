...
export function resolveHooksConfig(cfg: SelferConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: SelferConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-Selfer-token"] === "string"
      ? req.headers["x-Selfer-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
