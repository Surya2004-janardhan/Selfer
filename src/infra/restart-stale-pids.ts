 ...
 * returning control to the caller (typically `triggerSelferRestart` →
...
/**
 * Parse selfer gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("selfer")) {
 ...
 * Returns only PIDs that belong to selfer gateway processes (not the current process).
 */
