...
export type ConfigSchema = ReturnType<typeof SelferSchema.toJSONSchema>;
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = SelferSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "SelferConfig";
  const hints = applyDerivedTags(mapSensitivePaths(SelferSchema, "", buildBaseHints()));
  const next = {
