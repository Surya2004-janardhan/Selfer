...
function isWhatsAppConfigured(cfg: SelferConfig): boolean {
...
function isGenericChannelConfigured(cfg: SelferConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: SelferConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: SelferConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: SelferConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: SelferConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: SelferConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: SelferConfig, pluginId: string): SelferConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
