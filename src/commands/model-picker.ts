...
type PromptDefaultModelResult = { model?: string; config?: SelferConfig };
...
function resolveConfiguredModelRaw(cfg: SelferConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: SelferConfig): string[] {
...
export function applyPrimaryModel(cfg: SelferConfig, model: string): SelferConfig {
...
export function applyModelAllowlist(cfg: SelferConfig, models: string[]): SelferConfig {
  const defaults = cfg.agents?.defaults;
