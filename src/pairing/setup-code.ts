...
function resolveAuth(cfg: SelferConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.Selfer_GATEWAY_PASSWORD?.trim() ||
...
async function resolveGatewayTokenSecretRef(
  ...
  const hasTokenEnvCandidate = Boolean(
    env.Selfer_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim(),
  );
  ...
  if (mode !== "token") {
    const hasPasswordEnvCandidate = Boolean(
      env.Selfer_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim(),
    );
...
async function resolveGatewayPasswordSecretRef(
  ...
  if (mode !== "password") {
    const hasTokenCandidate =
      Boolean(env.Selfer_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim()) ||
