...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "Selfer.sandbox=1");
  args.push("--label", `Selfer.sessionKey=${params.scopeKey}`);
  args.push("--label", `Selfer.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `Selfer.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "Selfer.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`Selfer sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`Selfer sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("Selfer sandbox recreate --all");
}
