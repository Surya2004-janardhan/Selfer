...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("selfer node install"), formatCliCommand("selfer node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("selfer node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    SELFER_LOG_PREFIX: baseEnv.SELFER_LOG_PREFIX ?? "node",
