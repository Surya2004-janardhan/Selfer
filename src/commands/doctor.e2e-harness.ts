...
export const resolveSelferPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadSelferPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveSelferPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.Selfer_STATE_DIR;
  originalUpdateInProgress = process.env.Selfer_UPDATE_IN_PROGRESS;
  process.env.Selfer_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "Selfer-doctor-state-"));
  process.env.Selfer_STATE_DIR = tempStateDir;
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    delete process.env.Selfer_STATE_DIR;
  } else {
    process.env.Selfer_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.Selfer_UPDATE_IN_PROGRESS;
  } else {
    process.env.Selfer_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
