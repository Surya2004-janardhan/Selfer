...
function resolveAgentName(cfg: SelferConfig, agentId: string) {
...
function resolveAgentModel(cfg: SelferConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SelferConfig): AgentSummary[] {
